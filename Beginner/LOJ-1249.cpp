...
bool cmp (Data B, Data A)
{
    if((B.x*B.y*B.z) != (A.x * A.y * A.z))
        return (B.x*B.y*B.z) > (A.x*A.y*A.z);
    else if (B.pos!=A.pos) return B.pos<A.pos;
    return false;
}

int main()
{
    int t,T,i,j,n,a,b,c,temp1,temp2;
    scin(T);
    RUN_CASE(t,T)
    {
        scin(n);
        ...
        {
            cin>>ss[i];
            sc("%d %d %d",&ara[i].x,&ara[i].y,&ara[i].z);
            ara[i].pos = i;
        }
        sort(ara,ara+n,cmp);
        temp1=ara[0].x*ara[0].y*ara[0].z;
        temp2=ara[n-1].x*ara[n-1].y*ara[n-1].z;
        if(temp1 == temp2)
            pf("Case %d: no thief\n",t);
        else
            cout<<"Case "<<t<<": "<<ss[ara[0].pos]<<" took chocolate from "<<ss[ara[n-1].pos]<<endl;
    }
    return 0;
}
