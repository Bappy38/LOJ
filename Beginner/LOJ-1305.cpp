...
{
    int i,j,t,T;
    double ox,oy,ax,ay,bx,by,cx,cy,dx,dy,area1,area2,a,b,c,s,x,y,z,ans;
    scin(T);
    RUN_CASE(t,T)
    {
        sc("%lf %lf %lf %lf %lf %lf",&ax,&ay,&bx,&by,&cx,&cy);
        ox=(ax+cx)/2.0;
        oy=(ay+cy)/2.0;
        dx=(2.0*ox)-bx;
        dy=(2.0*oy)-by;
        a=sqrt((ax-cx)*(ax-cx)+(ay-cy)*(ay-cy))/2.0;
        x=a;
        b=sqrt((bx-dx)*(bx-dx)+(by-dy)*(by-dy))/2.0;
        y=b;
        c=sqrt((ax-bx)*(ax-bx)+(ay-by)*(ay-by));
        s=(a+b+c)/2.0;
        area1=sqrt(s*(s-a)*(s-b)*(s-c));
        z=sqrt((ax-dx)*(ax-dx)+(ay-dy)*(ay-dy));
        s=(x+y+z)/2.0;
        area2=sqrt(s*(s-x)*(s-y)*(s-z));
        ans=(2*area1)+(2*area2);
        pf("Case %d: %0.0f %0.0f %0.0f\n",t,dx,dy,ans);
    }
    return 0;
}
